...
class LambdaManager(object):
    ...
    def logs(self, func, start, end):
        ...
        group_name = f"/aws/lambda/{func.name}"
...
class CloudWatchEventSource(object):
    ...
    def add(self, func):
        ...
        if rule and self.delta(rule, params):
            log.debug(f"Updating cwe rule for {func.name}")
    ...
    def add(self, func):
        ...
        for arn in self.topic_arns:
            ...
            try:
                lambda_client.add_permission(
                    FunctionName=func.name,
                    StatementId=f'sns-topic-{topic_name}',
                    SourceArn=arn,
                    Action='lambda:InvokeFunction',
...
class ConfigRule(object):
    ...
    def delta(rule, params):
        ...
        if rule['Source'] != params['Source']:
            return True
        return rule.get('Description', '') != rule.get('Description', '')
